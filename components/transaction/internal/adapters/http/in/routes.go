...
func RegisterRoutesToApp(f *fiber.App, auth *middleware.AuthClient, th *TransactionHandler, oh *OperationHandler, ah *AssetRateHandler, bh *BalanceHandler, orh *OperationRouteHandler, trh *TransactionRouteHandler) {
	...
	f.Get("/v1/organizations/:organization_id/ledgers/:ledger_id/balances/:balance_id/history", auth.Authorize(midazName, "balances", "get"), http.ParseUUIDPathParameters("balance"), bh.GetBalanceAtTimestamp)
	...
	f.Get("/v1/organizations/:organization_id/ledgers/:ledger_id/accounts/:account_id/balances/history", auth.Authorize(midazName, "balances", "get"), http.ParseUUIDPathParameters("balance"), bh.GetAccountBalancesAtTimestamp)
